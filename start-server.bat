...
echo Starting Gomoku server on port 9527...
"%JAVA_EXE%" -Dfile.encoding=UTF-8 -cp "%JAR%;%LIB%\*" io.github.tissyboxc.gomoku.Main server 9527
if errorlevel 1 (
    echo.
    echo ERROR: Server failed to start. Check port 9527 and MySQL.
    pause
    exit /b 1
)
